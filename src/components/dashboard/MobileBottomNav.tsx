...
export default function MobileBottomNav() {
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-card/90 backdrop-blur-xl border-t border-border/40">
            <div className="flex items-center justify-around h-16 px-1">
                {TABS.map(({ text, icon: Icon, path }) => {
                    const isActive = pathname === path ||
                        (path !== '/candidate' && pathname.startsWith(path));
                    return (
                        <button
                            key={text}
