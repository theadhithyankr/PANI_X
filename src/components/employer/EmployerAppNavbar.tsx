...
function getPageName(pathname: string) {
    if (pathname.startsWith('/employer/candidates/')) return 'Candidate';
    if (pathname.startsWith('/employer/applications/')) return 'Application';
    return PAGE_NAMES[pathname] || 'Pani';
}

export default function EmployerAppNavbar() {
    ...
    return (
        <>
            {/* Mobile top bar */}
            <header className={`fixed top-0 left-0 right-0 z-40 lg:hidden transition-transform duration-300 ${hidden ? '-translate-y-full' : 'translate-y-0'}`}>
